...
def extraer_campos_sinpe(texto_ocr: str) -> dict:
    """
    Aplica Expresiones Regulares (Regex) sobre el texto extraído por el OCR 
    para aislar limpiamente los 4 campos obligatorios de comprobantes SINPE.
    """
    # 1. Regex para Número de comprobante / Referencia
    patron_comprobante = r'(?:Número\s*de\s*transacción|Comprobante|Referencia|FT)\s*[:\.]?\s*([A-Z0-9]{6,20})'
    
    # 2. Regex para Fecha del pago
    patron_fecha = r'(\d{2}/\d{2}/\d{4}(?:\s+\d{2}:\d{2}:\d{2}\s*(?:am|pm|AM|PM)?)?)'

    # 3. Regex para Teléfono origen
    patron_telefono = r'(?:Teléfono|Tel|Desde)\s*[:\.]?\s*(\d{8})'

    # 4. Regex para Monto (Símbolos variados previos + cantidad numérica)
    ...
    # Formatear el monto agregando el símbolo de Colones (₡)
    monto_limpio = None
    if match_monto:
...
if __name__ == "__main__":
    print("Módulo ocr_engine configurado con extracción de campos Regex para SINPE Móvil.")